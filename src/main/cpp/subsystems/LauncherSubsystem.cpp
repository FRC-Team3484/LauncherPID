...
void LauncherSubsystem::Periodic() {
    ...
    SmartDashboard::PutBoolean("Launched Sensor", GetLaunchSensor());
    ...
    if (_bottom_launcher_pid_controller !=NULL) {
        if (_target_speed == 0_rpm) {
            ...
            _bottom_launcher_pid_controller->SetReference(_target_speed.value(), rev::CANSparkMax::ControlType::kVelocity);
        }
    ...
    if (_top_launcher_pid_controller !=NULL) {
        if (_target_speed == 0_rpm) {
            ...
            _top_launcher_pid_controller->SetReference(_target_speed.value(), rev::CANSparkMax::ControlType::kVelocity);
        }
...
bool LauncherSubsystem::AtTargetRPM() {
    ...
    if (_counter_not_null_bottom) {
        if (_target_speed != 0_rpm) {
            bottom_at_speed = (_bottom_launcher_encoder->GetVelocity()-_target_speed.value()) * (_target_speed.value() >= 0 ? 1 : -1) > 0;
        } else {
    ...
    if (_counter_not_null_top) {
        if (_target_speed != 0_rpm) {
            top_at_speed = (_top_launcher_encoder->GetVelocity()-_target_speed.value()) * (_target_speed.value() >= 0 ? 1 : -1) > 0;
        } else {
            top_at_speed = true;
        }
    } else {
